...
const WorkItem = ({ title, active = false, onClick }: WorkItemProps) => {
  return (
    ...
    >
      ...
      />
      <p
        className={`text-[18px] leading-[96%] font-medium tracking-[0.01em] transition-all duration-300 ease-out ${active ? "text-[#1D1D1E]" : "text-[#6D6C7B]"} [leading-trim:cap-height] group-hover:translate-x-[2px] group-hover:text-[#1D1D1E]`}
      >
        {title}
      </p>
    </button>
